  ...
  \**************************************************/
			/***/ (
				...
				function flushObserver() {
					...
					queueMicrotask(() => {
						if (
							queuedMutations.length === queueLengthWhenTriggered
						) {
				...
				var mergeProxyTrap = {
					...
					set({ objects }, name, value, thisProxy) {
						...
						if (descriptor?.set && descriptor?.get)
							return (
								descriptor.set.call(thisProxy, value) || true
							);
...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`,
					...
					let directives2 = attributes
						.map(
							toTransformedAttributes(
								(newName, oldName) =>
									(transformedAttributeMap[newName] =
										oldName),
							),
						)
				...
				function safeParseBoolean(rawValue) {
					if (
						[1, "1", "true", "on", "yes", true].includes(rawValue)
					) {
						return true;
					}
					if (
						[0, "0", "false", "off", "no", false].includes(rawValue)
					) {
						return false;
					}
				...
				function plugin(callback) {
					let callbacks = Array.isArray(callback)
						? callback
						: [callback];
				...
				function applyBindingsObject(el, obj, original) {
					...
					let attributes = Object.entries(obj).map(
				...
				function cacheIdByNameOnElement(
					el,
					cacheKey,
					cleanup2,
					callback,
				...
				var teleportContainerDuringClone =
						...
						if (
							...
							if (Array.isArray(currentValue)) {
								...
								if (modifiers.includes("number")) {
									newValue = safeParseNumber(
										event.target.value,
									);
								} else if (modifiers.includes("boolean")) {
									newValue = safeParseBoolean(
										event.target.value,
									);
								} else {
					...
					evaluateItems((items) => {
						...
						for (let i = 0; i < adds.length; i++) {
							...
							clone2._x_refreshXForScope = (newScope) => {
								Object.entries(newScope).forEach(
									([key2, value]) => {

<?php
function tmdrImage($params = array()) {
    ...
    $imageTemplate = '';
    if( $isLazyLoad ){
        $imageTemplate .= '<div class="' . $blockName . '__image-container '.$containerClass.'"  ' . $imgContainerAttr . '><img data-src="' . $imgUrl . '" alt="' . esc_html($imgAlt) . '" src="' . $smallImageUrl . '" class="' . $blockName . '__image ratio-item lazyload" loading="lazy" ' . $imgAttr . '></div>';
    }else{
        $imageTemplate .= '<div class="' . $blockName . '__image-container '.$containerClass.'" ' . $imgContainerAttr . '><img alt="' . esc_html($imgAlt) . '" src="' . $imgUrl . '" class="' . $blockName . '__image  ratio-item" ' . $imgAttr . '></div>';
    }
    return $imageTemplate;
}

function getAcfImage($params = array()){
    $altParams ='';
    $urlParams='';
    $urlParamsSmall='';
    if ($params) {
...
function getImageTemplate($params = array()) {
    $imageTemplate = '';
    if($params){
        ...
        if($multiple && $isLazyLoad){
            $imageTemplate .= '
                <div class="' . $blockName . '__image-container '.$containerClass.'"  ' . $imgContainerAttr . '>
                    <picture>
                        <source media="(min-width:768px)" srcset="' . $imgUrl . '">
                        <img data-src="' . $imgUrl . '" alt="' . esc_html($imgAlt) . '" src="' . $imgMobileUrl . '" class="' . $blockName . '__image ratio-item lazyload" loading="lazy" ' . $imgAttr . '>
                    </picture>
                </div>';
        }elseif($multiple){
            $imageTemplate .= '
                <div class="' . $blockName . '__image-container '.$containerClass.'"  ' . $imgContainerAttr . '>
                    <picture>
                        <source media="(min-width:768px)" srcset="' . $imgUrl . '">
                        <img  alt="' . esc_html($imgAlt) . '" src="' . $imgMobileUrl . '" class="' . $blockName . '__image ratio-item" ' . $imgAttr . '>
                    </picture>
                </div>';
        }else{
            if( $isLazyLoad ){

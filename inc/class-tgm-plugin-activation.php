	...
	class TGM_Plugin_Activation
	{
		...
		public function inject_update_info($plugins)
		{
			...
			foreach ($plugins as $slug => $plugin) {
				...
				$repo_updates->response[$file_path]->new_version =
					$plugin["version"];
				$repo_updates->response[$file_path]->package =
					$plugin["source"];
				if (
		...
		public function maybe_adjust_source_dir(
			...
			if (
				!$this->is_tgmpa_page() ||
				!is_object($GLOBALS["wp_filesystem"])
			) {
				return $source;
			}
			...
			if (!empty($desired_slug)) {
				...
				if (!empty($subdir_name) && $subdir_name !== $desired_slug) {
					...
					if (
						true ===
						$GLOBALS["wp_filesystem"]->move($from_path, $to_path)
					) {
					...
					if (!$automatic) {
						...
						if (!isset($_POST["action"])) {
							echo '<div id="message" class="updated"><p>',
								esc_html(
									$this->strings["activated_successfully"]
								),
								" <strong>",
		...
		public function notices()
		{
			...
			if (!empty($message) || $total_required_action_count > 0) {
				...
				if (
					...
					$rendered =
						esc_html(
							$this->strings["notice_cannot_install_activate"]
						) .
						" " .
					...
					foreach ($message as $type => $plugin_group) {
						...
						foreach ($plugin_group as $plugin_slug) {
							$linked_plugins[] = $this->get_info_link(
								$plugin_slug
							);
						}
						unset($plugin_slug);
		...
		protected function get_admin_notice_class()
		{
			if (!empty($this->strings["nag_type"])) {
				return sanitize_html_class(
					strtolower($this->strings["nag_type"])
				);
			} else {
		...
		public function register($plugin)
		{
			...
			$plugin["source"] = empty($plugin["source"])
				? "repo"
				: $plugin["source"];
			$plugin["required"] = TGMPA_Utils::validate_bool(
				$plugin["required"]
			);
			...
			$plugin["source_type"] = $this->get_plugin_source_type(
				$plugin["source"]
			);
		...
		protected function get_plugin_source_type($source)
		{
			if (
				"repo" === $source ||
				preg_match(self::WP_REPO_REGEX, $source)
			) {
				return "repo";
		...
		public function is_plugin_installed($slug)
		{
			...
			return !empty(
				$installed_plugins[$this->plugins[$slug]["file_path"]]
			);
		}
	...
	class TGMPA_List_Table extends WP_List_Table
	{
		...
		protected function _gather_plugin_data()
		{
			...
			foreach ($plugins[$this->view_context] as $slug => $plugin) {
				...
				$table_data[$i]["status"] = $this->get_plugin_status_text(
					$slug
				);
				$table_data[$i][
					"installed_version"
		...
		protected function get_row_actions($item)
		{
			...
			if (!$this->tgmpa->is_plugin_installed($item["slug"])) {
				...
				if (
					false !==
						$this->tgmpa->does_plugin_have_update($item["slug"]) &&
				...
				foreach ($plugins_to_install as $slug) {
					...
					if (!empty($name) && !empty($source)) {
						...
						switch ($install_type) {
							...
							case "update":
								$file_paths[] =
									$this->tgmpa->plugins[$slug]["file_path"];
								$to_inject[$slug] =
									$this->tgmpa->plugins[$slug];
								$to_inject[$slug]["source"] = $source;
								break;
						}
					}
				...
				class TGMPA_Bulk_Installer extends Plugin_Upgrader
				{
					...
					public function run($options)
					{
						...
						if ($this->tgmpa->is_automatic) {
							if (
								"update" ===
								$this->skin->options["install_type"]
							) {
				...
				class TGMPA_Bulk_Installer_Skin extends Bulk_Upgrader_Skin
				{
					...
					public function add_strings()
					{
						if ("update" === $this->options["install_type"]) {
							...
							$this->upgrader->strings[
								"skin_before_update_header"
							] = __('Updating Plugin %1$s (%2$d/%3$d)', "tgmpa");
						} else {
							/* translators: 1: plugin name, 2: error message. */
							$this->upgrader->strings[
								"skin_update_failed_error"
							] = __(
							...
							if ($this->tgmpa->is_automatic) {
								...
								$this->upgrader->strings[
									"skin_update_successful"
								] = __('%1$s installed successfully.', "tgmpa");
								$this->upgrader->strings[
									"skin_upgrade_end"
								] = __(
